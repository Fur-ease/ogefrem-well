...
import { exportMonthlyDocx } from "@/server/services/report.service";
...
export async function POST(req: NextRequest) {
    try {
        ...
        const buffer = await exportMonthlyDocx(parsed.data.month);

        const [year, mon] = parsed.data.month.split("-").map(Number);
        const monthLabel = format(new Date(year, mon - 1, 1), "MMMM_yyyy");
        const filename = `OGEFREM_WELL_Report_${monthLabel}.docx`;
